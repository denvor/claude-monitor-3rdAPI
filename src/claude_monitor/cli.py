...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--theme",
        choices=["light", "dark", "classic", "auto"],
        default="auto",
    ...
    parser.add_argument(
        "--timezone",
        default="auto",
        help="display timezone [default: auto]",
    )
    parser.add_argument(
        "--csv",
        action="store_true",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    if args.clear:
        print("No saved configuration to clear (not using persistent settings)")
        return 0
    ...
    days_back = 1 if args.view == "summary" else 0
